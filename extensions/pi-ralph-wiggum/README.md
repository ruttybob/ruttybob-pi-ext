...
<img width="432" height="357" alt="Screenshot 2026-01-07 at 17 16 24" src="https://github.com/user-attachments/assets/68cdab11-76c6-4aed-9ea1-558cbb267ea6.png">

## How It Works

Each iteration of a Ralph loop runs as an **isolated child `pi` process**:

- No extensions loaded in the child session
- Fresh context window every iteration
- Progress and reflection files injected via `--append-system-prompt`
- Parent session manages state and monitors child output

This means:
- No context pollution across iterations
- No extension interference (including Ralph itself)
- Each iteration starts clean with just the task, progress, and reflection context

## Files

Each loop creates these files in `.ralph/`:

| File | Purpose |
|------|---------|
| `<name>.md` | Task file with goals and checklist |
| `<name>.progress.md` | Auto-injected progress tracker |
| `<name>.reflection.md` | Reflection log (used with `--reflect-every`) |
| `<name>.state.json` | Loop state (iteration, status, params) |
...
- Pi runs `ralph_start`, beginning iteration 1. Each iteration runs as an isolated child process.
