...
import { loadAnswerConfig, resolveExtractionModel } from "./config.js";
import {
	type Component,
	Editor,
	type EditorTheme,
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Resolve extraction model from config or fallback to session model
			const answerConfig = loadAnswerConfig(ctx.cwd);
			const { model: extractionModel, source: modelSource } = await resolveExtractionModel(
				answerConfig, ctx.model, ctx.modelRegistry, ctx.cwd,
			);

			// Run extraction with loader UI
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				const loader = new BorderedLoader(tui, theme, `Extracting questions using ${modelSource} (${extractionModel.id})...`);
	...
	pi.registerCommand("answer", {
		...
		handler: (_args: string, ctx) => answerHandler(ctx), // _args: raw command string, unused
	});
