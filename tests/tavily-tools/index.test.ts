...
describe("Extension entry point", () => {
  ...
  test("registers only session_start warning hook when TAVILY_API_KEY is missing", async () => {
    ...
    const { default: extension } = await import("../../extensions/tavily-tools/index.js");
  ...
  test("session_start, turn_end, and shutdown handlers accept context", async () => {
    ...
    expect(handlers["session_start"]!({}, ctx)).resolves.toBeUndefined();
    expect(handlers["turn_end"]!({}, ctx)).resolves.toBeUndefined();
